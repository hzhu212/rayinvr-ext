...
param_combinations_zh = {'OBC节点间距=25m，炮检距=25m', 'OBC节点间距=12.5m，炮检距=25m', 'OBC节点间距=25m，炮检距=12.5m', 'OBC节点间距=12.5m，炮检距=12.5m'};



plot_multi_lighting_analyze('s', false, 'survey_len', 5, 'bin_width', 0.001);



function [fig] = plot_multi_lighting_analyze(wave, refresh, varargin)
% 绘制各开采阶段的照明分析图
    ...
    ha = tight_subplot(4, 3, [.035 .015], [.07 .03], [.06 .10]);
    for ii = 1:numel(param_combinations)
        ...
        for jj = 1:numel(target_stages)
            ...
            plot_one_lighting_analysis(ax, data, bin_edges, sprintf('T%d', stage_idx-1), ilayer, ixtick, iytick);
...
function plot_one_lighting_analysis(ax, data, bin_edges, time_label, ilayer, ixtick, iytick)
% 根据传入的数据，绘制一副照明分析图
